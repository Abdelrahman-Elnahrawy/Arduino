...
digitalWrite(relay5,R5);} 
 ...
 analogWrite (relay6,R6);} 
  if ((char)payload[36] == 'D'){
int D = (((int)payload[38]-48)*1000+((int)payload[39]-48)*100+((int)payload[40]-48)*10+((int)payload[41]-48))*1000;} 

if ((char)payload[34] == 'A'){ X=2;}
if ((char)payload[34] == 'S'){ X=1;}
...
button =analogRead(A0);}
 ...
 if(now - button_time > 700 && button<1024){
  button_time = now;
if( button>=400  && button<500 ){R1=!R1; X=0;  
     digitalWrite(relay1,R1);}
 ...
 if (X == 2){
  if (digitalRead(pir)==HIGH) {
   ...
   if (now - lastMsg > 500) {
    lastMsg = now;
    ++value;
    snprintf (msg, MSG_BUFFER_SIZE, "R1:%d R2:%d R3:%d R4:%d R5:%d R6:%d T:%f D:%d X:%d  #%ld ",R1,R2,R3,R4,R5,R6,t,D,X,value);
