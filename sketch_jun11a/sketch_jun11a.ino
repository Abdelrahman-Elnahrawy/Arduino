...
if ((char)payload[0] == 'M'&&(char)payload[3] == '1'&&(char)payload[5] == 'n'){
  digitalWrite(M_P1, HIGH);
}
if ((char)payload[0] == 'M'&&(char)payload[3] == '1'&&(char)payload[5] == 'f'){
  digitalWrite(M_P1, LOW);
}



if ((char)payload[0] == 'M'&&(char)payload[3] == '2'&&(char)payload[5] == 'n'){
  digitalWrite(M_P2, HIGH);
}
if ((char)payload[0] == 'M'&&(char)payload[3] == '2'&&(char)payload[5] == 'f'){
  digitalWrite(M_P2, LOW);
}



if ((char)payload[0] == 'r'&&(char)payload[5] == 'H'&&(char)payload[7] == 'n'){
  digitalWrite(relayH, HIGH);
}
if ((char)payload[0] == 'r'&&(char)payload[5] == 'H'&&(char)payload[7] == 'f'){
...
if (now - lastMsg1 > 1000&& digitalRead(pir)==HIGH) {
 ...
 if (now - lastMsg2 > 700&& digitalRead(button)==HIGH) {
   ...
   if (now - lastMsg3 > 300&&analogRead(A0)<60) {
    lastMsg3 = now;
    ++value3;
    snprintf (msg, MSG_BUFFER_SIZE, "LASER PASS NO. #%d", value2);
    Serial.print("Publish message: ");
    Serial.println(msg);
     Serial.println("esp8266/LASER");
